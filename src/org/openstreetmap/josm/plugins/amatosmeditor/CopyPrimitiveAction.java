...
public class CopyPrimitiveAction extends BasePrimitiveAction
{
    ...
    protected boolean copyTags(List<Command> commands,OsmPrimitive osmPrim,OsmPrimitive amatPrim) {
		...
		for (String key : amatPrim.keySet()) {
			if(!key.startsWith("AMAT"))
				if(!tagsDoNotExport.contains(key)) {
					String amatValue = amatPrim.get(key);
					if(osmPrim.hasKey(key)) {
						String osmValue = osmPrim.get(key);
		...
		if(!amatPrim.hasKey(specialkey) && osmPrim.hasKey(specialkey)) {
			if(!"no".equals(osmPrim.get(specialkey))) {
		...
		for (String key : tagsRemoveIfAbsent) {
			if(!amatPrim.hasKey(key) && osmPrim.hasKey(key)) {
		...
		if(needConfirmation || true) {
			AMATComparePrimitiveDialog dialog = new AMATComparePrimitiveDialog(osmPrim,amatPrim,
		...
		if(!tagsToSet.isEmpty()) {
			...
			if(tagsConfirmed)
				commands.add(new ChangePropertyCommand(Collections.singletonList(osmPrim), tagsToSet));
			 
			//If change of tag loc_ref confirmed add a command for tag change
			if(locrefConfirmed) {
				if(tagsToSet.containsKey(loc_refText))
					commands.add(new ChangePropertyCommand(Collections.singletonList(osmPrim), 
							loc_refText,tagsToSet.get(loc_refText)));
			}			
		}
		
		return geomConfirmed;			//return if geometry must be copied too
    }

    protected void copyGeometry(List<Command> commands,Way osmWay,Way amatWay) {
...
//				if(!node.hasKeys() && !node.isConnectionNode()) {       //which one is better???
    ...
    protected void copyGeometry(List<Command> commands,Node osmNode,Node amatNode) {
		//Add a command to move OSM Node to AMAT Node position
		commands.add(new MoveCommand(osmNode, amatNode.getCoor()));
    }
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if( osmWay != null) {			
			...
			//copy tags and geometry, but only confirmed (or needed anyway)
			if(copyTags(commands,osmWay,amatWay))
				copyGeometry(commands,osmWay,amatWay);
		} else {
			...
			if( osmNode != null) {			
				...
				//copy tags and geometry, but only confirmed (or needed anyway)
				if(copyTags(commands,osmNode,amatNode))
					copyGeometry(commands,osmNode,amatNode);
			}
		}
