 ...
 * @author imi
 */
public class AMATSelectionManager implements MouseListener, MouseMotionListener, PropertyChangeListener {
	...
	private AMATNavigatableComponent dataComp;				//to use for getting data	
     ...
     */
    public AMATSelectionManager(SelectionEnded selectionEndedListener, boolean aspectRatio, 
    		NavigatableComponent navComp, AMATNavigatableComponent dataComp) {
    ...
    public Collection<OsmPrimitive> getSelectedObjects(boolean alt) {
        ...
        if (clicked) {
            ...
            OsmPrimitive osm = dataComp.getNearestNodeOrWay(center, OsmPrimitive.isSelectablePredicate, false);
            if (osm != null) {
                selection.add(osm);
            }
        } else {
            ...
            // ways
            for (Way w : dataComp.getCurrentDataSet().getWays()) {
